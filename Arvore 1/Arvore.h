...
void MostraNivel(int x, node *noh)
{
    x--;
    if (noh->right != NULL)
    {
        MostraNivel(x, noh->right);
    }
    if (noh->left != NULL)
    {
        MostraNivel(x, noh->left);
    }
    if (x == 0)
        printf("%d ", noh->value);
...
void MostraNivelT(int x,tree *tre){
    if(tre !=NULL){
    node *raiz=tre->root;
    printf("%d",raiz);
    }
...
int mediana(int tam,node *noh){
	int x=0;
	if (noh != NULL)
    {
        x+=mediana(tam,noh->left);
        if ((tam/2)+1 == x){
        printf("%d ", noh->value);
    }
        x+=mediana(tam,noh->right);
        if ((tam/2)+1 == x){
        printf("%d ", noh->value);
    }
        return x+1;
